...
**Shared deletion body (2026-06-01, shipped prod).** The per-table delete sequence is the single helper `deleteAllUserDataTx(tx, userId)` in [queries.ts](../src/lib/auth/queries.ts) — keeping ONE body is load-bearing so the two paths never drift on table coverage. Add a new per-user table there and both paths pick it up:
