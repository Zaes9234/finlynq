...
- **2026-05-10 — `delete_category` + envelope unification ([#237](https://github.com/finlynq/finlynq/issues/237))** (90 → 91 HTTP, 86 → 87 stdio; **BREAKING — server version 3.0.0 → 3.1.0**):
  ...
  - **Stdio `delete_category`.** Numeric `id` only — refuses `name` cleanly with the Phase-4 error template (no DEK available on the stdio transport). FK refuse-check is enforced unconditionally; `confirmation_token` parameter is accepted-but-not-required (single-shot delete).
  - **Envelope unification.** New `dataResponse(data)` helper in both `register-tools-pg.ts` and `register-core-tools.ts` (next to the existing `text()` / `txt()` / `err()` / `sqliteErr()` helpers). Every read tool that returned a raw value via `text(rows)` / `text({...inlineFields})` / `{ content: [...] }` is now `dataResponse(...)` — same JSON wire encoding, but always `{ success: true, data: <T> }`. Write tools that returned `{ success: true, message: ... }` (single text) or `{ success: true, transactionId, message, ... }` (multi-field) are normalized to `{ success: true, data: { ... } }`. Idempotency-key replay paths in `bulk_record_transactions` and `approve_staged_rows` were updated so cached responses written under the new shape replay as `{ success: true, data: { ...originalData, replayed: true } }`; pre-3.1.0 cached envelopes (still under the 72h window) get defensively wrapped on the lookup branch so the outer shape stays consistent across the rollover.
  ...
  - **OAuth scope classification.** `delete_category` falls into `mcp:write` by default (the "everything else is a write" rule in `src/lib/oauth-scopes.ts`). `preview_delete_category` matches the `preview_*` read prefix and lands in `mcp:read`. Defense-in-depth verified: a read-only token cannot see `delete_category` in `tools/list`.
  - **Auto-annotations.** `delete_category` is `destructiveHint: true` / `readOnlyHint: false` automatically (inferred by `delete_*` prefix in `mcp-server/auto-annotations.ts`); `preview_delete_category` is `readOnlyHint: true` (inferred by `preview_*`).
  - **Anthropic Connectors Directory submission impact.** Clients connected mid-rollout will see `serverInfo.version: 3.1.0` after deploy. The directory entry still says `3.0.0` until the founder updates the form (Page 3 — `MCP server version`). Per the issue source: "engineering side — ship and merge." The PR description includes a verbatim "What's new" / changelog block for the founder to copy into the form.
...
- **2026-05-04 — `record_transfer` honors `receivedAmount` on same-account cash-sleeve forex ([#125](https://github.com/finlynq/finlynq/issues/125))** (no tool count change; HTTP only — stdio refuses investment-account writes already):
  ...
  - **Heuristic:** when both sides resolve to cash sleeves (`portfolio_holdings.symbol_ct IS NULL`) inside the SAME account, AND the holding names carry divergent ISO-4217 suffixes (regex `[\s\-_]([A-Z]{3})$`), AND the caller passed `receivedAmount` and omitted `destQuantity`, the destination leg's `amount`, `quantity`, and `enteredFxRate` derive from `receivedAmount` instead of falling back to `sentAmount`. Source leg is unchanged.
  - **Disambiguation:** name your cash sleeves with the trailing `- XXX` ISO code (`Cash - USD`, `Cash - CAD`). Plain `Cash` skips the heuristic and `receivedAmount` is ignored as before — falling back to the safe "same conceptual currency" path. Symbol-bearing holdings are excluded by the cash-sleeve gate so a user-named `My ETF - USD` cannot trip the heuristic.
  - **Out of scope:** `updateTransferPair` and `createTransferPairViaSql` carry the same dual-write semantics (L1080-L1100, L1213-L1214 / L1761-L1768, L1864 in `transfer.ts`); follow-up issue should mirror the heuristic. Stdio `record_transfer` is unchanged — it already refuses investment-account writes per the no-DEK / no-`portfolioHolding`-surface carve-out.
  - Cross-refs: #92 (strict find-only on destination cash sleeve, preserved), #94 (UI Transfer dialog, separate surface), four-check transfer-pair rule (relaxed for same-account in-kind already — this stays inside that envelope).

- **2026-05-04 — `get_portfolio_analysis` gains `account_id` / `account` filter ([#123](https://github.com/finlynq/finlynq/issues/123))** (no tool count change; HTTP fully wired, stdio schema-symmetric only):
  - **HTTP `get_portfolio_analysis` accepts two new optional filters.** `account_id: number` is the FK fast-path (skips fuzzy matching, cheap ownership check via `SELECT 1 FROM accounts ...`). `account: string` goes through the existing `resolveAccountStrict` waterfall and rejects low-confidence matches (token-overlap-≥3 gate, same as `record_transaction`). When both are passed, `account_id` wins — mirrors the precedence in every existing write tool.
  - **Implementation rides the existing `holding_accounts` JOIN grain (issue #25).** No new joins; the filter is a single `AND ph.account_id = ${scopeAccountId}` predicate on the existing `phRaw` query, plus a defensive `if (scopeAccountId != null && !info) continue` guard in the per-row metrics loop. The `holding_accounts.qty` / `cost_basis` cached columns stay untouched — aggregators continue reading live `SUM(transactions.quantity)` per CLAUDE.md "Portfolio aggregator".
  - **Warnings array contract preserved.** Account-scope misses join the existing `symbols` warnings as plain strings: `"account_id=N: not found"` (FK miss / cross-tenant), `"<input>: no matching account (did you mean <suggestion> id=N?)"` (low-confidence fuzzy), `"<input>: no matching account"` (no candidate at all). Empty-holdings short-circuit instead of erroring — keeps callers monomorphic, same shape as all-`symbols`-unmatched.
  - **Stdio `get_portfolio_analysis` schema gets `account_id` / `account` for transport symmetry**, but the handler still refuses the call wholesale under Stream D Phase 4 (no DEK on stdio = no decrypted account/holding names). Documented in the schema description as accepted-but-unused. A single client can target either transport with the same payload — and on stdio still gets the standard Phase-4 refusal pointing to HTTP MCP or the web UI.
  - Cross-refs: #86 (warnings-array pattern + `holding_id`-keyed `phMap` reused here), #25 (`holding_accounts` JOIN grain rationale), #99 (account-move refusal — the new filter scopes the read, doesn't move anything). Future follow-ups: same filter on `get_portfolio_performance` and `analyze_holding` are out of scope here; `analyze_holding` already has `holdingId` for disambiguation per #86.

- **2026-05-04 — `get_portfolio_analysis` `symbols` matcher tightened + mirrored on stdio ([#124](https://github.com/finlynq/finlynq/issues/124))** (no tool count change; HTTP + stdio):
  - **HTTP `symbols` semantic switched from OR-within-entry to AND-within-entry across `name + symbol + account`.** Previously `symbols: ["VCN.TO (TFSA)"]` would match anything containing `VCN.TO` OR `TFSA`, sweeping in unrelated cash sleeves named `TFSA-CAD`. The matcher now requires every whitespace/paren-separated token in a single entry to appear in the row's `name + symbol + account` haystack. Across multiple entries the union (OR) semantic is unchanged. The full-string substring fast path (`haystack.includes(raw)`) stays for the cheap single-token case (`"VCN.TO"` still matches without paying tokenization cost). The `account` field is now part of the haystack — that's what makes `"VCN.TO (TFSA)"` correctly resolve.
  - **Stdio `get_portfolio_analysis` gains the `symbols` parameter** with the same AND-within-entry semantic, and now returns `warnings` to mirror the HTTP shape. The stdio query already SELECTs `ph.name`/`ph.symbol`/`a.name as account_name`, so the in-process matcher uses those directly. Field-shape agnostic — once issue #131 lands the upstream SELECT will populate name/symbol from `name_ct`/`symbol_ct`.
  - Empty/whitespace-only entries (e.g. `"()"` tokenizes to `[]`) are skipped so they surface in `warnings` rather than vacuously matching every row.
  - Cross-ref: extends #86 (which introduced the `warnings` array + `holding_id` keying); #86's other invariants (`accumulate()` / `aggregateHoldings()` / `phMap` keyed by `holding_id`, response row count) are unchanged.
...
**Realized-gain skip on paired cash-leg rows ([#128](https://github.com/finlynq/finlynq/issues/128)).** The same paired cash-leg row that the cost-basis aggregator JOINs to (above) is also EXCLUDED from the FIFO realized-gain sell branch in three of the four aggregators: REST `/api/portfolio/overview`, MCP HTTP `accumulate()` (powering `get_portfolio_performance`), and MCP HTTP `analyze_holding`. The skip predicate is `trade_link_id IS NOT NULL AND amount = 0`. Without it, a cash-sleeve row generated as the cash side of a paired buy (qty<0, amount=0, trade_link_id=<uuid>) would fall into the qty<0 sell branch with `sellAmt=0` and `sellQty=N`, producing a phantom realized loss of `0 - N * avgCost` ≈ `-N` (cash sleeves track 1 unit per dollar so `avgCost ≈ 1`). The predicate is conjunctive — both `trade_link_id != null` AND `amount = 0` — so a legitimate cash withdrawal (no link, amount<0) keeps its existing realized-gain treatment. Stdio `get_portfolio_performance` doesn't need the skip: its cost_basis branch gates on `amount<0` and proceeds on `amount>0`, so an amount=0 row contributes 0 to both columns and `realizedPnL = proceeds - costBasis = 0` for the sleeve. Stdio `analyze_holding` doesn't compute realized gain at all (qty<0 branch only nets `totalShares`).
...
- `get_portfolio_analysis` — exposes `id: <int>` per holding in the `holdings[]` array (HTTP + stdio). Issue #86 (2026-05-01): the HTTP path now keys its in-memory aggregator Map by `holding_id` (not display name), so two holdings sharing a name across accounts (e.g. VUN.TO in TFSA + RRSP) come through as separate rows instead of being silently merged. The `symbols` filter (HTTP + stdio after #124, 2026-05-04) substring-matches the row's `name + symbol + account` combination — within a single entry, ALL whitespace/paren-separated tokens must appear in the haystack (AND); across entries the result is the union (OR). So `"VCN.TO (TFSA)"` requires both `vcn.to` AND `tfsa` to appear (no longer leaks `TFSA-CAD` cash sleeves), and `["VTI", "MNT.TO"]` returns the union. Unmatched entries surface in a top-level `warnings: ["BNO: no matching holding found"]` array.
...
Post Stream D Phase 4 (prod + dev, 2026-05-03 / 2026-05-07), the 12 stdio create/update tools across the 6 in-scope tables (`add_account`, `update_account`, `add_loan`, `update_loan`, `add_subscription`, `update_subscription`, `bulk_add_subscriptions`, `add_goal`, `update_goal`, `create_category`, `add_portfolio_holding`, `update_portfolio_holding`) refuse cleanly via `streamDRefuse()` — they can't compute the encrypted name siblings without a DEK. Permanent-vs-fixable tradeoff analysis in [stdio-mcp-write-decision.md](stdio-mcp-write-decision.md) (Option A, FINLYNQ-5); the refusal message text cites this doc so users hitting the error land on the explanation.
